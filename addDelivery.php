<?php
    ...
    $station = $date = $reg = $mid = $prem = "";
    $station_err = $date_err = $reg_err = $mid_err = $prem_err = "";
    $success_msg = "";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty(trim($_POST['regular']))) {
            $reg_err = "Please enter gallons.";
            $hasError = true;
        }
        elseif (!preg_match("/^\d{1,11}?$/", $_POST['regular'])) {
        ...
        if (empty(trim($_POST['midgrade']))) {
            $mid_err = "Please enter gallons.";
            $hasError = true;
        }
        elseif (!preg_match("/^\d{1,11}?$/", $_POST['midgrade'])) {
        ...
        if (empty(trim($_POST['premium']))) {
            $prem_err = "Please enter gallons.";
            $hasError = true;
        }
        elseif (!preg_match("/^\d{1,11}?$/", $_POST['premium'])) {
        ...
        if (!$hasError) {
                ...
                $sql = "INSERT INTO delivery (StationID, Date, Reg, Mid, Prem) 
                    VALUES ('$stationid', '$date', '$regular', '$midgrade', '$premium') ";

                if ($conn->query($sql) === TRUE) {
                    $success_msg = "
                    <div class='w3-margin w3-padding w3-pale-blue'>
                        <strong>New Delivery added successfully!</strong><br>
                        <br>
                        Station ID: $stationid<br>
                        Date: $date<br>
                        Regular: $regular gal<br>
                        MidGrade: $midgrade gal<br>
                        Premium: $premium gal<br>
                    </div>";
...
<body class="w3-theme-d5">
    <div class="w3-theme-d5">
		...
		<form action="addDelivery.php" method="POST" class="w3-margin">
